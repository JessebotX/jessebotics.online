...
export default function Prompt({ link, text, children }: PromptProps) {
  ...
  return (
    <>
      ...
      <div className="flex mt-2">
        <h2 className="text-xl font-bold"><Link href={link ? link : '/'}>~{link}</Link> $ </h2>
        <h2 className="text-xl ms-2">{input}</h2>
        <span id="prompt-cursor" className="ms-[0.25ch] w-[0.7rem] bg-black"></span>
      </div>
      <article id="prompt-content" className="mt-10 hidden">
        {children}
      </article>
    </>
  )
